...
function encodeConsonantWord(word) {
  ...
  let a = arr.findIndex(letter => letter.match(/[aeiou]/));
  let strAfterVowel = arr.slice(a).join("");
  let strBeforeVowel = arr.slice(0, a).join("");

  console.log(strAfterVowel + "-" + strBeforeVowel + "ay");

  return strAfterVowel + "-" + strBeforeVowel + "ay"; // replace this!
}
...
function encodeWord(word) {
   return word.match(/^[aeiou]/) ? encodeVowelWord(word) : encodeConsonantWord(word);
...
function encodeText(text) {
  ...
  return arr.map(word=>encodeWord(word)).join(' ');
}
...
div.textContent = encodeText(text);
out.appendChild(div)



}
document.getElementById('submit').addEventListener('click', ()=> encodeUserText());
/*
